...
for (let i = 0; i < totalChars; i++) {
    if (uniqueRows.indexOf(nodes[i].race) > -1) continue;
    uniqueRows.push(nodes[i].race);

    let row = {};
    row.race = nodes[i].race;
    row.amount = 0;
    row.percentage = 0;
...
for (let i = 0; i < distribution.length; i++) {
    let rowName = distribution[i].race;
    for (let j = 0; j < totalChars; j++) {
        if (rowName === nodes[j].race) distribution[i].amount += 1;
    }
